...
public abstract class AbstractTree<E> implements Tree<E> {
	...
	public Iterable<Position<E>> breadthfirstStack() {
		List<Position<E>> snapshot = new ArrayList<>();
		if (!isEmpty()) {
			Stack<Position<E>> stack = new Stack<>();
			/* Comienzo empujando la raiz */
			stack.push(root());

			while (!stack.isEmpty()) {
				Position<E> p = stack.pop(); // Quitar del stack
				snapshot.add(p);
				/*
				 * hijo drecho se empuja primero para que el hijo izquierdo se procese primero,
				 * manteniendo el orden que debe
				 */
				for (Position<E> c : children(p)) {
					stack.push(c);
				}
	...
	public boolean duplicate() {
		...
		for (Position<E> p : positions()) {
			if (elements.contains(p.getElement())) {
				return true;
			}
	...
	public Iterable<Position<E>> listGreaterAncestor(){
	    Position<E> deepest = null;
	    int maxDepth = -1;
	    for (Position<E> p : positions()) {
	        if (isExternal(p) && depth(p) > maxDepth) {
	            maxDepth = depth(p);
	            deepest = p;
	        }
	    }
	    return ancestor(deepest);
	}
	/**
