...
public class SinglyLinkedList<E> implements Cloneable {
  ...
  //Agrega un elemento en la lista en la posicion p
  public void add(int p, E e) throws IndexOutOfBoundsException{
	  Node<E> current = head;
	  if ((0>p)||(p>size)) {
		  throw new IndexOutOfBoundsException("p esta fuera de rango");
	  }
	  if (p==0) {
		  addFirst(e);
	  }else if(p== size){
  ...
  //Remueve el elemento E de la lista, retorna null si no encuentra
  public E remove(E e) {
      Node<E> current = head;
      Node<E> removido = current;
      while (current != null) {					
      	if (current.getNext().getElement().equals(e)) {	
            removido = current.getNext();
            current.setNext(removido.getNext());
            size--;
  ...
  //Remueve y retorna la posicion p
  public E remove(int p) throws IndexOutOfBoundsException{
	  ...
	  if (p==0) {
          ...
          E removedElement = current.getNext().getElement();
