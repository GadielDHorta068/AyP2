...
public class LinkedBinaryTree<E> extends AbstractBinaryTree<E> {
    ...
    public List<Position<E>> path(Position<E> from, Position<E> to) {
        ...
        int i = fromAncestors.size() - 1;
        int j = toAncestors.size() - 1;

        //Busco el comun
        while (i >= 0 && j >= 0 && fromAncestors.get(i) == toAncestors.get(j)) {
            i--;
            j--;
        }
        //camino de from al comun
        for (int k = 0; k <= i + 1; k++) {
            path.add(fromAncestors.get(k));
        }
        //camino de comun a to
        for (int k = j; k >= 0; k--) {
            path.add(toAncestors.get(k));
        }
        return path;
    }
